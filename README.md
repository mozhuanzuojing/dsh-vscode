...
- ⏳ **信号下沉**：审批/澄清问题下沉到 VSCode 弹窗/输入框应答；可选 OpenViking 语义检索补充上下文（`[Repo Recall]`，直连其 MCP server，无需 ov CLI）
- 🧠 **编辑器联动**：① 编辑器右键「用 DSH 解释/修复/重构」时注入 `[User Input]` 上下文（工作区/相对路径/文件类型/行号/按大小给内容；界面直接输入不注入，避免历史堆积脏上下文）② agent 写文件后**先预览 diff、确认/忽略再应用**（不覆盖未保存修改，30s 回合窗口）③ 上下文实时反映当前选区
