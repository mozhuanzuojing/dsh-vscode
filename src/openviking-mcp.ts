...
export async function mcpFind(url: string, query: string, limit = 3): Promise<McpFindResult> {
  ...
  try {
    if (!initialized) {
      await mcpPost(url, { jsonrpc: '2.0', id: 1, method: 'initialize', params: { protocolVersion: '2025-06-18', capabilities: {}, clientInfo: { name: 'dsh-awakening', version: '0.7.10' } } }, ac.signal);
